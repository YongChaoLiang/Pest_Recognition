...
All modifications are for user experience and the limitation of device, including:
* Using tensorboard logger
* Changing Dataset IP102
* Reset batch size as 128
...
def get_args_parser():
    parser = argparse.ArgumentParser('Biformer training and evaluation script', add_help=False)  ## myself
    ...
    parser.add_argument('--teacher-model', default='biformer_tiny_best', type=str, metavar='MODEL',
    ...
    # * Finetuning params
    parser.add_argument('--finetune', default=r'./Output/best.pth',
                        help='finetune from checkpoint')

    # Dataset parameters
    parser.add_argument('--data-path', default='../Data/pest/classification', type=str,
                        help='dataset path')
    parser.add_argument('--data-set', default='MYDS', choices=['CIFAR', 'IMNET', 'INAT', 'INAT19', "MYDS"],
                        type=str, help='Image Net dataset path')
    parser.add_argument('--nb-classes', default=102,
                        help='Num of classes when use MYDS')
    ...
    parser.add_argument('--log_dir', default='./Output/log',
                        help='log where to save, empty for no saving')
...
def main(args):
    ...
    model = biformer.biformer_tiny(pretrained=False, nm_class=args.nb_classes, pretrained_dir=args.finetune)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser('Biformer_whole training and evaluation script', parents=[get_args_parser()])
