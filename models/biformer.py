"""
BiFormer impl.
Modified from https://github.com/rayleizhu/BiFormer/blob/public_release/models/biformer.py
...
@register_model
def biformer_tiny(pretrained=False, nm_class=1000, pretrained_dir =r'.\Pre-trained\biformer_tiny_best.pth', pretrained_cfg=None,
                  pretrained_cfg_overlay=None, **kwargs):
    model = BiFormer(
        depth=[2, 2, 8, 2],
        num_classes=nm_class,
    ...
    if pretrained:
        print(os.path.exists(pretrained_dir))
...
@register_model
def biformer_small(pretrained=False, nm_class = 1000, pretrained_dir =r'E:\Code\Pytorch_gpu\demo_1\BiFormer\Pre-trained\biformer_small_best.pth', pretrained_cfg=None,
                   pretrained_cfg_overlay=None, **kwargs):
    model = BiFormer(
        depth=[4, 4, 18, 4],
        num_classes=nm_class,
    ...
    if pretrained:
        ...
        if os.path.exists(pretrained_dir):
            ...
            print("Downloading {}".format(model_key))
            url = model_urls[model_key]
            checkpoint = torch.hub.load_state_dict_from_url(url=url, map_location="cpu", check_hash=True, file_name=f"{model_key}.pth")
        model.load_state_dict(checkpoint["model"])
